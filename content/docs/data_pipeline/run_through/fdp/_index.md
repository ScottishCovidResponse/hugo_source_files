...
- generate a working *config.yaml* file (see [DPI API Example 1]({{% ref "/docs/data_pipeline/run_through/example1" %}}))
  - globbing (`*` and `**` replaced with all matching objects, all components listed), specific version numbers, and any variables (*e.g.* `{CONFIG_DIR}`) is replaced with true values
